...
def build_flutter_deb(version, features):
    ...
    os.system(
        'cp ../res/com.sodesk.SODesk.policy tmpdeb/usr/share/polkit-1/actions/')
    os.system(
...
def main():
    ...
    if windows:
        ...
        os.system('mv target/release/sodesk.exe target/release/SODesk.exe')
        pa = os.environ.get('P')
        if pa:
            os.system(
            ...
            print('Not signed')
        os.system(f'cp -rf target/release/SODesk.exe sodesk-{version}-win7-install.exe')
        ...
        if flutter:
            ...
            if osx:
                os.system(
                    'strip target/release/bundle/osx/SODesk.app/Contents/MacOS/sodesk')
                os.system(
                    'cp libsciter.dylib target/release/bundle/osx/SODesk.app/Contents/MacOS/')
                # https://github.com/sindresorhus/create-dmg
                os.system('/bin/rm -rf *.dmg')
                plist = "target/release/bundle/osx/SODesk.app/Contents/Info.plist"
                txt = open(plist).read()
    ...
    #rcodesign sign --p12-file ~/.p12/sodesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/SODesk.app/Contents/MacOS/sodesk
    #rcodesign sign --p12-file ~/.p12/sodesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/SODesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/sodesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/SODesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/SODesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/SODesk.app
    '''.format(pa))
                os.system('create-dmg target/release/bundle/osx/SODesk.app')
                os.rename('SODesk %s.dmg' %
    ...
    # verify:  spctl -a -t exec -v /Applications/SODesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
